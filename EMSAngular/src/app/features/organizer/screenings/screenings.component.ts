...
import { endAfterStart, futureDateTime, notBlank } from '../../../shared/validators/form-validators';
...
export class ScreeningsComponent implements OnInit {
  ...
  // Screenings define the event's window rather than being bounded by it, so the only
  // time rules are: start in the future and end after start.
  protected form = this.fb.nonNullable.group({
    screen: ['', [Validators.required, notBlank, Validators.minLength(1), Validators.maxLength(50)]],
    startTime: ['', [Validators.required, futureDateTime]],
