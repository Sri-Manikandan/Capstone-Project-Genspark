...
def test_settings_read_from_env(monkeypatch):
    ...
    monkeypatch.setenv("JWT_SIGNING_KEY", "signing-key")
    monkeypatch.setenv("JWT_ISSUER", "EMSApi")
    monkeypatch.setenv("JWT_AUDIENCE", "EMSClient")
    s = Settings()
    ...
    assert s.jwt_signing_key == "signing-key"
    assert s.jwt_issuer == "EMSApi"
    assert s.jwt_audience == "EMSClient"
